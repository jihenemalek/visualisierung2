...
bool ModelClass::Initialize(ID3D11Device* device, char* modelFilename, WCHAR* textureFilename, HWND m_hwnd)
{
	...
	Vesseltree::Root *tree0 = Vesseltree::Parser::parseDocument("Resources/vesselTree5.xml", m_hwnd);
	Sampling* sampling0 = new Sampling;
	//sampling0->downsample(tree0, 0.1, 1); 

	mesh0 = new Mesh;
	mesh0->calculateMesh(tree0);
	AdaptiveSubdivision* subdiv = new AdaptiveSubdivision;

	//mesh0->triangles = subdiv->Subdivide(mesh0->triangles,0.1,2);
